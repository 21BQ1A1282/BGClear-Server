    ...
    public Map<String, Object> verifyPayment(String razorpayOrderId) throws RazorpayException {
        ...
        try{
            ...
            if(orderInfo.get("status").toString().equalsIgnoreCase("paid")){
                OrderEntity existingOrder = orderRepository.findByOrderId(razorpayOrderId).orElseThrow(()-> new RazorpayException("Order not found with id: " + razorpayOrderId));
            ...
            throw new RazorpayException("Error while verifying payment: " + e.getMessage());
        }
        return returnValue;
    }
}
